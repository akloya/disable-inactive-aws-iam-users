...
This is a sample template for inactive-iam-users - Below is a brief explanation of what we have generated for you:
...
```bash
sam logs -n HelloWorldFunction --stack-name inactive-iam-users --tail
```
...
```bash
aws cloudformation delete-stack --stack-name inactive-iam-users
```
